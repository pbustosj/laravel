...
<div class="table90">
	<div class="row">
		@if(!empty($message))
		<?php
			$typeOfMessage = substr($message,0,1);
			$colorMessage = $typeOfMessage == 'S' ? 'green' : 'red';
			$bodyMessage = substr($message,1,strlen($message)-1);

		?>
		<script>

			$(document).ready(function(){
				Materialize.toast('<?= $bodyMessage ?>', 5000, '<?= $colorMessage ?> rounded');
			})
		</script>
		{{$message = null }}
		@endif
		<form class="col s12" id="formSaveUser" method="POST" action="{{ url('user/save') }}">
			{{ csrf_field() }}
			<div class="row">
			...
			<div class="row center-align">
				...
				<a id="btnGuardar" class="btn-large waves-effect waves-light green ">Crear Usuario</a>
			</div>
